...
export const LINTER_CONFIGS: Record<LinterType, LinterConfig> = {
  ...
  biome: {
    ...
    env_vars: {},
    supported_extensions: ['.js', '.jsx', '.ts', '.tsx', '.json'],
  ...
  prettier: {
    ...
    env_vars: {},
    supported_extensions: ['.js', '.jsx', '.ts', '.tsx', '.json', '.md', '.css'],
    config_files: ['.prettierrc', '.prettierrc.json', '.prettierrc.yml'],
    output_parsers: {
      text: output => output,
    },
    timeout_ms: 30000,
  ...
  jshint: {
    ...
    output_parsers: { json: o => JSON.parse(o) },
    timeout_ms: 30000,
    fix_supported: false,
  },
  pylint: {
