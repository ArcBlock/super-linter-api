...
EXAMPLES:
    $0                      # Build optimized image (default)
    $0 -t full --push      # Build full image and push
    $0 -v latest            # Build optimized image, tag as latest
    $0 --skip-tests         # Build optimized without running tests

SIZE COMPARISON:
    Optimized: ~4.5GB  (Layer-optimized, 18 linters, production-ready) [RECOMMENDED]
    Full: ~6GB         (All 50+ linters, Super-linter base)
...
build_superlinter_image() {
    ...
    log_warn "This may take 5-8 minutes and requires ~4.5GB disk space"
